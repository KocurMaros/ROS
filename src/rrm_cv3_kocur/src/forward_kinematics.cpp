...
void ForwardKinematics::jointCallback(const sensor_msgs::JointState::ConstPtr& msg)
{
    ...
    Eigen::MatrixXd T0 = createTz(joint_state_.position[0]) * createTz(joint_state_.position[1]) * createTz(joint_state_.position[2]) * createTz(joint_state_.position[3]) ;//+ createRy(joint_state_.position[3] * createTz(L3));
