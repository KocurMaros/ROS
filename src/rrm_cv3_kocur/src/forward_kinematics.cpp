...
void ForwardKinematics::jointCallback(const sensor_msgs::JointState::ConstPtr& msg)
{
    ...
    dh = DH(0,0,0,joint_state_.position[0])*DH(0,90*M_PI/180,L1,joint_state_.position[1])*DH(L2+joint_state_.position[2],90*M_PI/180,L2+joint_state_.position[2],0)*DH(0,0,L3,-M_PI+joint_state_.position[3]);
