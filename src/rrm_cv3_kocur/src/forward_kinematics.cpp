...
void ForwardKinematics::jointCallback(const sensor_msgs::JointState::ConstPtr& msg)
{
    ...
    Eigen::MatrixXd T0 = createRz(joint_state_.position[0]) * createRz(joint_state_.position[1]) * createRz(joint_state_.position[2]) ;//+ createRy(joint_state_.position[3] * createTz(L3));
