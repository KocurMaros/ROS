#include "rrm_cv3_ridilla/forward_kinematics.h"
...
void ForwardKinematics::jointCallback(const sensor_msgs::JointState::ConstPtr& msg)
{
    ...
    Eigen::MatrixXd j3 = createRz(joint_state_.position[0]) * createTz(L1) * createRy(joint_state_.position[1]) * createTz(L2) * createTz(L3+joint_state_.position[2]);

    // convert rotation matrix to tf matrix
    tf::Matrix3x3 tf3d;
    tf::Matrix3x3 tf3d_j2;
    tf::Matrix3x3 tf3d_j3;
    ...
    tf3d_j2.setValue(static_cast<double>(j2(0,0)), static_cast<double>(j2(0,1)), static_cast<double>(j2(0,2)),
    ...
    // Convert to quternion
    tf3d_j2.getRotation(orientation2_);

    tf3d_j3.setValue(static_cast<double>(j3(0,0)), static_cast<double>(j3(0,1)), static_cast<double>(j3(0,2)),
    ...
    // Convert to quternion
    tf3d_j3.getRotation(orientation3_);
